...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!reel.video_path || !reel.video_path.startsWith(`${user.id}/`)) {
      ...
      // Optional poster upload if your renderer returns one
      if (result.posterJpegBuffer) {
        const upPoster = await supabase.storage
          .from('reels-private')
          .upload(paths.privatePoster, result.posterJpegBuffer, { contentType: 'image/jpeg', upsert: true });
        if (upPoster.error) throw upPoster.error;
      }

      await updateReel(supabase, reelId, {
        video_path: paths.privateVideo,
        poster_path: result.posterJpegBuffer ? paths.privatePoster : reel.poster_path || null,
